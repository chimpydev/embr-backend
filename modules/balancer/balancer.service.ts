...
import { embrFarmService } from '../embr/embr-farm.service';
...
export class BalancerService {
    ...
    public async cachePools(): Promise<GqlBalancerPool[]> {
        ...
        const farms = await embrFarmService.getEmbrFarms();
        ...
        const decoratedPools = filteredWithOnChainBalances.map((pool): GqlBalancerPool => {
            ...
            const {
                ...
                ? embrFarmService.calculateFarmApr(farm, farmTvl, blocksPerYear, embrPrice)
                : { items: [], thirdPartyApr: '0', embrApr: '0' };
